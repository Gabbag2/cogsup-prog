...
def stroop():
    ...
    for j in range(2):
        for i in range(10):
            ...
            type_trial = "True" if (word == "red" and color == C_RED) or (word == "blue" and color == C_BLUE) or (word == "green" and color == C_GREEN) or (word == "orange" and color == C_ORANGE) else "False"
            accuracy = "True" if (type_trial == "True" and key == K_RIGHT) or (type_trial == "False" and key == K_LEFT) else "False"
            ...
            affichage = stimuli.TextLine("Your answer is " + accuracy + " and your reaction time is " + str(rt) + " ms", text_colour=C_BLACK, text_size=30)
            affichage.present(True, True)
            exp.clock.wait(2000)
